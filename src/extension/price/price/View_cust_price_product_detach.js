...
let extension = {
  ...
  tableAction: 'View_cust_price_product_detach', //指定某张表的权限(这里填写表名,默认不用填写)
  ...
  methods: {
    ...
    detachSelectedData(){
      ...
      this.$confirm('Are you sure to detach the selected products ?', 'warning', {
        ...
        this.http.post("api/View_cust_price_product_detach/detachProductFromGroup", { mainData: ids }, true)
            .then((x) => {
              if (!x.status) {
    ...
    detachAll(){
      ...
      this.$confirm('Are you sure to detach all the products ?', 'warning', {
        ...
        this.http.post("api/View_cust_price_product_detach/detachAll", mainData, true)
            .then((x) => {
              if (!x.status) {
                this.$Message.error(x.message);
                return;
